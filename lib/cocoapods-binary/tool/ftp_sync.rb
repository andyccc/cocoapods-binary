...
#      filenames = @connection.nlst(source)
      filenames = @connection.list(source)
      log("filenames is " + filenames)
      return filenames
  end
  ...
  def put(path, des_path)

      return @connection.put(path, des_path)
  end
  
  def local_file(path)

      return @connection.local_file(path)
  end
  ...
  private
    def connect!
      @connection = Net::FTP.new(@server, @port)
